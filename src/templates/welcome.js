...
var out='<img src="https://invie78.fr/images/logo.jpg" alt="logo" />\n\nParis, le '+((new Date).toISOString().substr(0,10))+'\n\nBonjour **'+ (getProperty(data, "representant.prenom", ""))+' '+ (getProperty(data, "representant.nom", ""))+'**,\n\nL\'enregistrement de votre société **'+ (getProperty(data, "nom", ""))+'** a bien été pris en compte en date du '+ (getProperty(data, "date_creation", ""))+'.\n\n';return out.replace(/(<([^>]+)>)/gi, "");
};
...
var out='<img src="https://invie78.fr/images/logo.jpg" alt="logo" /><p>Paris, le '+((new Date).toISOString().substr(0,10))+'</p><p>Bonjour <strong>'+ (getProperty(data, "representant.prenom", ""))+' '+ (getProperty(data, "representant.nom", ""))+'</strong>,</p><p>L\'enregistrement de votre société <strong>'+ (getProperty(data, "nom", ""))+'</strong> a bien été pris en compte en date du '+ (getProperty(data, "date_creation", ""))+'.</p>';return out;
};
...
var out='L\'enregistrement de '+ (getProperty(data, "nom", ""))+' a bien été reçu';return out;
}
...
var out='"'+ (getProperty(data, "representant.prenom", ""))+' '+ (getProperty(data, "representant.nom", ""))+'" <'+ (getProperty(data, "representant.email", ""))+'>';return out;
}
...
var out='attestation';return out;
}
