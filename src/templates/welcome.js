 ...
 * @return {String}
 */
export const html = (data) => ('<img src="https://invie78.fr/images/logo.jpg" alt="logo" /><p>Paris, le '+((new Date).toISOString().substr(0,10))+'</p><p>Bonjour <strong>'+(data.representant.prenom)+' '+(data.representant.nom)+'</strong>,</p><p>L\'enregistrement de votre société <strong>'+(data.nom)+'</strong> a bien été pris en compte en date du '+(data.date_creation)+'.</p>');;
 ...
 * @return {String}
 */
export const to = (data) => ('"'+(data.representant.prenom)+' '+(data.representant.nom)+'" <'+(data.representant.email)+'>');
